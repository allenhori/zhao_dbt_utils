{#
  ...
  STANDALONE VARIANT: this is the copy-paste-into-your-own-project
  version (Option B in README.md's Install section) -- drop this file
  directly into your own project's macros/ folder, no packages.yml, no
  dbt deps. Its macros call each other WITHOUT the zhao_utils.
  namespace prefix internally, which is what makes bare calls
  (`{{ wref(...) }}`, no prefix at all) work once this file lives in
  your own project. This is a genuinely different file from the real
  dbt package's macros/zhao_ref.sql (self-namespaced, required for
  proper package-mode installs) -- not just a copy, the internal calls
  are deliberately different. Confirmed via testing that these two
  variants are NOT interchangeable: self-namespaced internal calls
  break in copy-paste mode (there's no zhao_utils package to resolve
  against), and bare internal calls break in real package-mode
  installs (even sibling macros within the same installed package
  can't call each other bare -- dbt's own constraint, not a bug here).
  ...
  *itself*, describing how far back/forward *it* needs to read from its
  ...
  Usage note: call these bare -- `{{ wref(...) }}`, no namespace
  prefix -- since this file lives directly in your own project once
  copied in, not in an installed package. Confirmed via testing.
#}
...
{% macro zhao_window_start(upstream_name, expand_back=none) %}
  {%- if not execute -%}
    {{ return('') }}
  {%- endif -%}
  {%- set resolved = _zhao_resolve(upstream_name, 'lookback', expand_back) -%}
...
{% macro zhao_window_end(upstream_name, expand_forward=none) %}
  {%- if not execute -%}
    {{ return('') }}
  {%- endif -%}
  {%- set resolved = _zhao_resolve(upstream_name, 'lookahead', expand_forward) -%}
...
{% macro wref(upstream_name, expand_back=none, expand_forward=none) %}
  ...
  {%- endif -%}
  {%- set start = zhao_window_start(upstream_name, expand_back) -%}
  {%- set end = zhao_window_end(upstream_name, expand_forward) -%}
(
